...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await MaterialsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await MaterialsDBApi.findAll(
    req.query, { currentUser }
  );
  if (filetype && filetype === 'csv') {
    const fields = ['id','material_name','supplier',
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await MaterialsDBApi.findBy(
    { id: req.params.id },
  );
